...
twitter_data.each do | hash |
  hash.each do |name, description|
    puts "#{name}:#{description["number of followers"]}"
  end
end

twitter_data.each do | hash |
  hash.each do |name, description|
    puts "#{name}'s latest tweet was #{description["latest tweet"].length} characters long."
  end
end
...
twitter_data.each do | hash |
  hash.each do |name, description|
    total_characters = 0
    20.times do |i|
      if description["last twenty tweets"][i] != nil
        total_characters += description["last twenty tweets"][i].length
      end
    end
    puts "#{name}'s used #{total_characters} characters in their last twenty tweets."
  end
end
...
twitter_data.each do | hash |
  hash.each do |name, description|
    if description["number of followers"] != nil && description["number of followers"] >= followers_count
      most_followers = name
      followers_count = description["number of followers"]
    end
    if description["number of friends"] != nil && description["number of friends"] >= friends_count
      most_friends = name
      friends_count = description["number of friends"]
    end
    if description["number of tweets"] != nil && description["number of tweets"] >= tweets_count
      most_tweets = name
      tweets_count = description["number of tweets"]
    end
  end
end
puts "#{most_followers}'s has the most followers."
puts "#{most_friends}'s has the most friends."
puts "#{most_tweets}'s has the greatest number of tweets."
...
twitter_data.each do | hash |
  hash.each do |name, description|
    if description["description"] != nil
      users_with_description << name
    end
    if description["location"] != nil
      users_with_location << name
    end
  end
end

puts "Users with description listed in twitter_data:"
puts users_with_description.join(', ')

puts "Users with location listed in twitter_data:"
puts users_with_location.join(', ')
