...
people.each do |person|
  puts person.first if person.last["children"].nil? == false
end
puts 'What are the names of the people who do not have children?'
people.each do |person|
  puts person.first if person.last["children"].nil?
end
...
puts sets_of_people[0].keys.count
...
sets_of_people.each do |set|
  puts set["Ruby Hessel"]["phone"] unless set["Ruby Hessel"].nil?
end
puts "What are the names of Dr. Sigrid Nader's children?"
sets_of_people.each do |set|
  puts set["Dr. Sigrid Nader"]["children"] unless set["Dr. Sigrid Nader"].nil?
end
puts "What is Bernard Feil's child's name?"
sets_of_people.each do |set|
  puts set["Bernard Feil"]["children"] unless set["Bernard Feil"].nil?
end
puts 'What company does Casandra Kemmer work for?'
sets_of_people.each do |set|
  puts set["Casandra Kemmer"]["company"] unless set["Casandra Kemmer"].nil?
end
